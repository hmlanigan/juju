...
func (s *baseRelationSuite) setRelationSuspended(c *tc.C, uuid string, suspended bool, reason string) {
	s.query(c, `
UPDATE relation
SET suspended = ?, suspended_reason = ?
WHERE uuid = ?`, suspended, reason, uuid)
}
