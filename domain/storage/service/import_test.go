...
func (s *importSuite) TestImportFilesystemsCAAS(c *tc.C) {
	...
	s.state.EXPECT().GetStoragePoolProvidersByNames(gomock.Any(), tc.Bind(tc.SameContents, []string{"kubernetes"})).Return(map[string]string{
	...
	// Act
	err := s.service.ImportFilesystemsCAAS(c.Context(), "test-namespace", params)
