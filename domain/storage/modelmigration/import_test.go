...
func (s *importSuite) TestRegisterImport(c *tc.C) {
	...
	RegisterImport(s.coordinator,
		...
		func(context.Context, func(context.Context, storage.FilesystemModelMigration) error) error {
			return nil
		},
...
func (s *importSuite) TestImportFilesystemsCAAS(c *tc.C) {
	...
	s.noopStoragePoolImport()
	s.service.EXPECT().ImportFilesystemsCAAS(gomock.Any(), "moveme", tc.Bind(tc.SameContents, []domainstorage.ImportFilesystemParams{{
		ID:                "fs-1",
		SizeInMiB:         1024,
		StorageInstanceID: "multi-fs/1",
		PoolName:          "kubernetes",
